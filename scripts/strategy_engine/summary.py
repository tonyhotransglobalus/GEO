...
def build_rescience_pass(
    ...
    if (
        ...
        priority_actions["P1"].append("Publish llms.txt and llms-full.txt at the site root.")
    ...
    geo_methods = []
    if citability_data.get("average_citability_score", 0) < 60:
        geo_methods.extend(
            [
                {"method": "Cite Sources", "impact": "+40%", "recommendation": "Add authoritative citations to key service and explainer sections."},
                {"method": "Statistics Addition", "impact": "+37%", "recommendation": "Add concrete numbers and sourced facts to answer blocks."},
                {"method": "Fluency Optimization", "impact": "+15-30%", "recommendation": "Tighten paragraphs and improve extractability for AI answers."},
            ]
        )

    if len(page_data.get("h1_tags", [])) != 1:
        geo_methods.append(
            {"method": "Answer-first Structure", "impact": "Structural", "recommendation": "Use one H1 and clearer H2/H3 sections with direct answers near the top."}
        )

    if not llms_validation.get("exists"):
        geo_methods.append(
            {"method": "FAQ Schema", "impact": "+40% AI visibility", "recommendation": "Add FAQ content and FAQPage schema on priority service pages."}
        )
    ...
    if not llms_validation.get("exists"):
        summary_parts.append("The biggest structural gap is the missing llms.txt guidance layer.")
    if citability_data.get("average_citability_score", 0) < 50:
...
def build_findings(
    ...
    if not llms_exists:
        add_finding(
            ...
            summary=(
                "The site does not expose llms.txt guidance files at the root, so AI systems receive no direct guidance "
                "about priority pages, document structure, or recommended crawl targets."
            ),
            leadership_impact=(
                "Without a machine-readable guidance layer, the company has less control over how AI systems discover and "
                "prioritize its most valuable content."
            ),
            marketing_action=(
                "Publish llms.txt and llms-full.txt that point AI systems to the pages most aligned with brand messaging, "
                "core services, and thought-leadership assets."
            ),
            developer_action=(
                "Create and deploy root-level llms.txt files, keep them versioned with the site, and update them whenever "
                "priority URLs or content architecture changes."
            ),
            observed_evidence=(
                "The live fetch did not find llms.txt at the expected root path, so no AI guidance document is currently published."
            ),
        )
    elif not llms_valid:
        add_finding(
            ...
            summary=(
                ...
                "so AI systems still lack reliable crawl instructions."
            ),
            leadership_impact=(
                "A malformed guidance layer creates false confidence because the file exists but does not yet provide dependable machine-readable direction."
            ),
            marketing_action=(
...
def build_executive_summary(
    ...
    if not llms_exists:
        summary.append("The most urgent structural issue is the missing llms.txt guidance layer.")
    elif not llms_valid:
        summary.append("The most urgent structural issue is that llms.txt exists but the guidance layer is malformed.")
    if heading_count != 1:
